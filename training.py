...
def train_model(df, img_dir, name,
    ...
    logger.info('Length of training df is ' + str(df_train.shape[0]))
    logger.info('Length of validation df is ' + str(df_val.shape[0]))
    logger.info(df_val['Sample'].unique())
    ...
    logger.info('Num of parameters is ' + str(sum(p.numel() for p in network.parameters())))
    # logger.info([p.numel() for p in network.parameters()])
    ...
    for epoch in tqdm(range(n_epochs)):  # loop over the dataset multiple times
        ...
        now2 = time.time()
        logger.info('Epoch time is ' + str(now2 - then2))

    now = time.time()

    logger.info('Training time is ' + str(now - then))
    ...
    if save_directory != False:
        ...
        logger.info('mse is ' + str(((val_labels - val_outputs) ** 2).mean()))
        logger.info(pearsonr(val_labels, val_outputs))
        ...
        if save_visualizations == True:
            ...
            dest = os.path.join(str(save_directory), name, "validation/worst_fits")

            key_list = df_val.head(4)['Key'].tolist()
            logger.info('Saving worst validation fits')
            for key in key_list:
                ####### Visualizations
                img_path = os.path.join(img_dir, key)
                logger.info('Saving ' + img_path)
                visualize_sample(network, img_path, dest)

            # Then the best fits
            dest = os.path.join(str(save_directory), name, "validation/best_fits/")

            key_list = df_val.tail(4)['Key'].tolist()
            logger.info('Saving best validation fits')
            for key in key_list:
                ####### Visualizations
                img_path = os.path.join(img_dir, key)
                logger.info('Saving ' + img_path)
            ...
            df_train['Error'] = abs(
                df_train['Stiffness'] - df_train['Predicted Stiffness']) ** 2  # / df_train['Stiffness'] #
            df_train = df_train.sort_values('Error', ascending=False)

            # First the worst fits
            dest = os.path.join(str(save_directory), name, "training/worst_fits/")
            ...
            # Then the best fits
            dest = os.path.join(str(save_directory), name, "training/best_fits/")
    ...
    if save_directory == False:
        if save_visualizations == True:
            logger.info('No visualizations were saved because no save_directory was specified')
