...
def generate_STIFMap(dapi, collagen, name, step, models,
    ...
    logger.info('Image shape is ' + str(im.shape))
    ...
    # 20231023 using ceil instead of + 1 to properly set the x_range and y_range
    x_range = ceil(im.shape[2] / step)
    y_range = ceil(im.shape[1] / step)

    # Create the list that will eventually be returned
    out = []

    logger.info('Num squares in x direction is ' + str(x_range))
    logger.info('Num squares in y direction is ' + str(y_range))
    ...
    for model in tqdm(models, desc="Predicting..."):
        ...
        for i in range(len(x)):
            ...
            # print(min(step * y[i], mask.shape[0]-1), min(step * x[i], mask.shape[1]-1))
            masked_value = mask[step * y[i], step * x[i]]
            # and add it to the list
            masked_value_tracker.append(masked_value)

            # If a model is outside the masked region, then we don't want to use it
            if masked_value != 0:
                # Otherwise, (1) get the square
                im_sub = im[:, step * y[i]:(step * y[i] + step), step * x[i]:(step * x[i] + step)].copy()
        ...
        # Fit the last group of squares too, but only if there's something there
        if len(squares) != 0:
            outputs_sub = network(torch.stack(squares))
            outputs_sub = torch.reshape(outputs_sub, (-1,))
            outputs += list(outputs_sub.detach().cpu().numpy().flatten())
        ...
        now2 = time.time()
        logger.debug('Time taken to predict squares is ' + str(now2 - then2))
        ...
        if save_dir:
            np.save(os.path.join(str(save_dir), name + '_model_' + model_name + '.npy'), z_out)

    # Save memory
    del im

    now = time.time()
    logger.info('Total time taken is ' + str(now - then))
...
def collagen_paint(dapi, collagen, z, name, step,
    ...
    """
    Pseudo-color the collagen image to indicate the predicted stiffness of the collagen fibers
    ...
    x = np.arange(0, x_range * step, step)
    y = np.arange(0, y_range * step, step)
    ...
    if save_dir:
        plt.imsave(os.path.join(str(save_dir), name + '_STIFMap.png'), collagen_colored)
    ...
    if save_dir:
        plt.savefig(os.path.join(str(save_dir), name + "_colorbar.png"))

    now = time.time()
    logger.info('Time taken is ' + str(now - then))
...
#     logger.info('Spearman correlation between stain and predicted stiffness is ' + str(z_stain_spearman[0]))
#
...
#     logger.info('Spearman correlation between stain and collagen is ' + str(collagen_stain_spearman[0]))
#
...
#     logger.info('Spearman correlation between stain and dapi is ' + str(dapi_stain_spearman[0]))
#
#     now = time.time()
#     logger.info('time taken is ' + str(now - then))
#
